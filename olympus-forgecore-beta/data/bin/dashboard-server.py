...
AUTH_CONFIG = CFG / "auth.json"
AUTH_SECRET = ST / ".auth-secret"
...
STORE_ID = re.compile(r"^[a-z0-9]+(?:-[a-z0-9]+)*$")
...
def default_global_config(v1_config=None):
    ...
    return {
        ...
        "communityStore": {
            "provider": "github-store-repository",
            "repository": "",
        },
        "runtime": {
...
def validate_global_config(data):
    ...
    release_provider = str(releases.get("provider") or "").strip()
    if release_provider not in {"github-releases", "github-releases"}:
        raise ValueError("Global releases.provider must be github-releases.")
    release_repository = str(releases.get("repository") or "").strip()
    if release_repository and not REPO.fullmatch(release_repository):
        raise ValueError("Global releases.repository must look like owner/repository.")
    normalized_releases = {"provider": "github-releases", "repository": release_repository}
    ...
    store_provider = str(store.get("provider") or "").strip()
    if store_provider not in {"github-store-repository", "github-store-repository"}:
        raise ValueError("Global communityStore.provider must be github-store-repository.")
    store_repository = str(store.get("repository") or "").strip()
    if store_repository and not REPO.fullmatch(store_repository):
        raise ValueError("Global communityStore.repository must look like owner/repository.")
    normalized_store = {"provider": "github-store-repository", "repository": store_repository}
    if "authRef" in store:
...
def ensure_global_config():
    ...
    ST.mkdir(parents=True, exist_ok=True)
    candidate = read_json(GLOBAL_CONFIG) if GLOBAL_CONFIG.exists() else default_global_config()
    cleanup = candidate.get("cleanup") if isinstance(candidate, dict) else None
    if isinstance(cleanup, dict) and "buildkitKeepStorageGB" not in cleanup:
...
def olympus_releases_integration(global_config=None):
    ...
    return {
        ...
        "configurationReady": bool(str(releases.get("repository") or "").strip() and auth_ref and SECRET_ENV_REF.fullmatch(auth_ref)),
    }
...
def save_olympus_releases_integration(data):
    require_keys(
        ...
        "Release destination integration",
    )
    repository = str(data.get("repository") or "").strip()
    if not REPO.fullmatch(repository):
        raise ValueError("Release destination repository must look like owner/repository.")
    auth_ref = str(data.get("authRef") or "").strip()
    if auth_ref:
        auth_ref = _validate_secret_ref(auth_ref, "Release destination credential reference")
    signing_enabled = data.get("signingEnabled")
    if not isinstance(signing_enabled, bool):
        raise ValueError("Release destination signingEnabled must be true or false.")

    releases = {"provider": "github-releases", "repository": repository}
    if auth_ref:
        releases["authRef"] = auth_ref
    if signing_enabled:
        encoding = str(data.get("signingEncoding") or "").strip()
        if encoding not in {"base64", "plain"}:
            raise ValueError("Release destination signing encoding must be base64 or plain.")
        releases["signing"] = {
            "provider": "sigstore-key",
            "privateKeyRef": _validate_secret_ref(data.get("privateKeyRef"), "Release destination private key reference"),
            "publicKeyRef": _validate_secret_ref(data.get("publicKeyRef"), "Release destination public key reference"),
            "passwordRef": _validate_secret_ref(data.get("passwordRef"), "Release destination password reference"),
            "encoding": encoding,
        }
    ...
    append_activity("integration", f"Release destination integration updated for {repository}")
...
def community_app_store_integration(global_config=None):
    ...
    return {
        ...
        "authRef": auth_ref,
        "credentialReferenceConfigured": bool(auth_ref and SECRET_ENV_REF.fullmatch(auth_ref)),
        "configurationReady": bool(str(store.get("repository") or "").strip() and auth_ref and SECRET_ENV_REF.fullmatch(auth_ref)),
    }
...
def save_community_app_store_integration(data):
    require_keys(
        ...
        {"repository", "authRef"},
        {"repository", "authRef"},
        "Store destination integration",
    )
    repository = str(data.get("repository") or "").strip()
    if not REPO.fullmatch(repository):
        raise ValueError("Store destination repository must look like owner/repository.")
    auth_ref = str(data.get("authRef") or "").strip()
    if auth_ref:
        auth_ref = _validate_secret_ref(auth_ref, "Store destination credential reference")
    ...
    global_config = validate_global_config(global_config)
    write_json_atomic(GLOBAL_CONFIG, global_config)
    append_activity("integration", f"Store destination updated for {repository}")
    return community_app_store_integration(global_config)

def clear_release_destination():
    global_config = ensure_global_config()
    global_config["releases"] = {"provider": "github-releases", "repository": ""}
    global_config = validate_global_config(global_config)
    write_json_atomic(GLOBAL_CONFIG, global_config)
    append_activity("integration", "Release destination removed")
    return olympus_releases_integration(global_config)

def clear_store_destination():
    global_config = ensure_global_config()
    global_config["communityStore"] = {"provider": "github-store-repository", "repository": ""}
    global_config = validate_global_config(global_config)
    write_json_atomic(GLOBAL_CONFIG, global_config)
    append_activity("integration", "Store destination removed")
    return community_app_store_integration(global_config)
...
def settings():
    ...
    return {
        ...
        "release_destination": olympus_releases_integration(global_config),
        "store_destination": community_app_store_integration(global_config),
...
def ensure_control_plane():
    ...
    default_path = PD / "default-delivery.json"
...
def delete_publish_preset(preset_id):
    preset_id = validate_slug(preset_id, "Publish preset id")
    if preset_id == "default-delivery":
        raise RunnerConflictError("The built-in default-delivery policy can be edited but not deleted.")
...
def validate_app(data):
    ...
    if store_id and not STORE_ID.fullmatch(store_id):
        raise ValueError("storeId must be a lowercase slug when configured.")
    ...
    if publish is not None:
        ...
        if (preset.get("store") or {}).get("enabled") and not store_id:
            raise ValueError("Store App ID is required when the selected publish preset enables the Community App Store.")
...
def _resolved_store_directory(app, store):
    ...
    if not STORE_ID.fullmatch(directory):
        raise ValueError("Store directory template must resolve to a valid app id.")
...
def validate_publish_job(app, preset, global_snapshot, job_type, ref):
    ...
    if not release_enabled and not store_enabled:
        raise ValueError("Publish jobs require a release or store destination to be enabled.")
    ...
    if release_enabled:
        ...
        if releases.get("provider") != "github-releases":
            raise ValueError("Global releases.provider must be github-releases.")
        repository = str(releases.get("repository") or "")
        if not REPO.fullmatch(repository):
            raise ValueError("Global releases.repository must look like owner/repository.")
    ...
    if store_enabled:
        if app.get("build", {}).get("executor") != "forgecore-native":
            raise ValueError("Store publishing requires the forgecore-native executor.")
        if not release_enabled:
            raise ValueError("Store publishing requires release publishing to be enabled.")
        if release.get("channel") != "stable":
            raise ValueError("Store publishing accepts stable release presets only.")
        if (app.get("package") or {}).get("format") != "umbrel":
            raise ValueError("Store publishing currently requires an Umbrel package.")
...
def publish_preflight(app_id, ref):
    ...
    if release_enabled:
        release_configured = releases.get("provider") == "github-releases" and bool(REPO.fullmatch(str(releases.get("repository") or ""))) and bool(SECRET_ENV_REF.fullmatch(str(releases.get("authRef") or "")))
        add("release-config", "Release destination", release_configured, str(releases.get("repository") or "Not configured"))
        add(
            "release-credential",
            "Release credential",
    ...
    if store_enabled:
        ...
        store_configured = community.get("provider") == "github-store-repository" and bool(REPO.fullmatch(str(community.get("repository") or ""))) and bool(SECRET_ENV_REF.fullmatch(str(community.get("authRef") or "")))
        ...
        add("store-id", "Store App ID", bool((app.get("identity") or {}).get("storeId")), str((app.get("identity") or {}).get("storeId") or "Not configured"))
        add("store-config", "Store destination", store_configured, str(community.get("repository") or "Not configured"))
        add(
            "store-credential",
            "Store credential",
...
def status():
    ...
    x["bootstrap_beta55_seen"] = "ForgeCore 0.1.0-beta.55 runner bootstrap started" in bootstrap_text
...
AUTH_COOKIE = "forgecore_session"
AUTH_ITERATIONS = 310000
AUTH_SESSION_SECONDS = 60 * 60 * 24 * 7
AUTH_USERNAME = re.compile(r"^[A-Za-z0-9_.-]{3,64}$")

def _auth_secret():
    ST.mkdir(parents=True, exist_ok=True)
    if AUTH_SECRET.exists():
        value = AUTH_SECRET.read_text(encoding="utf-8").strip()
        if re.fullmatch(r"[0-9a-f]{64}", value):
            return bytes.fromhex(value)
    secret = secrets.token_bytes(32)
    tmp = AUTH_SECRET.with_name("." + AUTH_SECRET.name + ".tmp")
    tmp.write_text(secret.hex() + "\n", encoding="utf-8")
    os.chmod(tmp, 0o600)
    inherit_owner(tmp, ST)
    os.replace(tmp, AUTH_SECRET)
    return secret

def _password_hash(password, salt, iterations=AUTH_ITERATIONS):
...
def auth_account():
    if not AUTH_CONFIG.exists():
        return None
    account = read_json(AUTH_CONFIG)
    if account.get("kind") != "ForgeCoreAccount" or account.get("schemaVersion") != 1:
        raise ValueError("ForgeCore account configuration is invalid.")
    return account

def create_auth_account(data):
    if AUTH_CONFIG.exists():
        raise RunnerConflictError("ForgeCore account setup is already complete.")
    require_keys(data, {"username", "password"}, {"username", "password", "displayName"}, "Account setup")
    username = str(data.get("username") or "").strip()
    display_name = str(data.get("displayName") or "").strip() or username
    password = str(data.get("password") or "")
    if not AUTH_USERNAME.fullmatch(username):
    ...
    write_json_atomic(AUTH_CONFIG, account)
    append_activity("security", f"ForgeCore account created for {username}")
    return account

def verify_auth_account(username, password):
    account = auth_account()
    if account is None or not hmac.compare_digest(str(account.get("username") or ""), str(username or "")):
        return None
    password_meta = account.get("password") or {}
    try:
        iterations = int(password_meta.get("iterations") or AUTH_ITERATIONS)
        salt = str(password_meta["salt"])
        expected = str(password_meta["hash"])
        actual = _password_hash(str(password or ""), salt, iterations)
    except (KeyError, TypeError, ValueError):
        return None
    return account if hmac.compare_digest(actual, expected) else None
...
def issue_session(username):
    payload = json.dumps(
        {"u": username, "exp": int(time.time()) + AUTH_SESSION_SECONDS},
        separators=(",", ":"),
        sort_keys=True,
    ).encode("utf-8")
...
def request_user(handler):
    ...
    try:
        ...
        expected = _b64url(hmac.new(_auth_secret(), encoded.encode("ascii"), hashlib.sha256).digest())
        ...
        if int(payload.get("exp") or 0) < int(time.time()):
            return None
        account = auth_account()
        if account is None or not hmac.compare_digest(str(payload.get("u") or ""), str(account.get("username") or "")):
            return None
...
def auth_status(handler):
    account = auth_account()
    user = request_user(handler) if account is not None else None
    return {
        "setupRequired": account is None,
        "authenticated": user is not None,
        "user": user,
    }

def auth_cookie(token):
    return f"{AUTH_COOKIE}={token}; Path=/; HttpOnly; SameSite=Strict; Max-Age={AUTH_SESSION_SECONDS}"

def expired_auth_cookie():
    return f"{AUTH_COOKIE}=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0"
...
class Handler(BaseHTTPRequestHandler):
    server_version = "ForgeCoreDashboard/0.3"
    ...
    def send_html(self):
        try:
            ...
            self.send_error(503, "ForgeCore dashboard runtime unavailable")
            return
        self.send_response(200)
    ...
    def require_auth(self):
        ...
        if user is None:
            self.send_json(401, {"error": "Authentication required", "authenticationRequired": True})
            return None
        return user
    ...
    def do_GET(self):
        ...
        if path == "/icon.svg":
            ...
            return
        if path == "/health":
            self.send_json(200, {"ok": True, "web_runtime_version": RUNTIME_VERSION, "web_build": WEB_BUILD})
            return
        if path == "/api/auth/status":
            self.send_json(200, auth_status(self))
            return
        ...
        if path == "/api/status":
            ...
            item = get_app(path[len("/api/apps/"):])
            ...
            item = get_publish_preset(path[len("/api/publish-presets/"):])
            ...
            item = get_job(path[len("/api/jobs/"):])
            self.send_json(200, {"job": job_api_view(item)}) if item else self.send_json(404, {"error": "Job not found"})
        elif path in {"/api/integrations/release-destination", "/api/integrations/olympus-releases"}:
            self.send_json(200, {"integration": olympus_releases_integration()})
        elif path in {"/api/integrations/store-destination", "/api/integrations/community-app-store"}:
            ...
            if not kind:
                ...
                return
            log_path = resolve_log(kind)
            if not log_path:
                self.send_json(404, {"error": "Unknown log source"})
                return
            self.send_json(200, {"kind": kind, "text": tail_text(log_path)})
        else:
    ...
    def do_POST(self):
        ...
        try:
            if path == "/api/auth/setup":
                account = create_auth_account(data)
                token = issue_session(account["username"])
                self.send_json(201, {"ok": True, "user": _public_account(account)}, {"Set-Cookie": auth_cookie(token)})
                return
            if path == "/api/auth/login":
                require_keys(data, {"username", "password"}, {"username", "password"}, "Login")
                account = verify_auth_account(data.get("username"), data.get("password"))
                if account is None:
                    self.send_json(401, {"error": "Invalid username or password."})
                    return
                append_activity("security", f"ForgeCore login for {account['username']}")
                self.send_json(200, {"ok": True, "user": _public_account(account)}, {"Set-Cookie": auth_cookie(issue_session(account["username"]))})
                return
            if path == "/api/auth/logout":
                self.send_json(200, {"ok": True}, {"Set-Cookie": expired_auth_cookie()})
                return
    ...
    def do_DELETE(self):
        ...
        try:
            ...
            if path.startswith("/api/apps/"):
                ...
                self.send_json(200, {"ok": True, "integration": clear_release_destination()})
            elif path in {"/api/integrations/store-destination", "/api/integrations/community-app-store"}:
                self.send_json(200, {"ok": True, "integration": clear_store_destination()})
            else:
                self.send_json(404, {"error": "Not found"})
        except RunnerConflictError as exc:
            self.send_json(409, {"error": str(exc)})
