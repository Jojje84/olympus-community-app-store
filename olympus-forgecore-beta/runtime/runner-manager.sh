...
REQUEST_DIR="${STATE_DIR}/runner-requests"
RESET_DIR="${STATE_DIR}/runner-resets"
RUNNER_ROOT="${STORAGE_ROOT}/runners"
LOG_DIR="${STORAGE_ROOT}/logs"
DIST_ROOT="${FORGECORE_RUNNER_DIST_ROOT:-/home/runner}"

mkdir -p "${REQUEST_DIR}" "${RESET_DIR}" "${RUNNER_ROOT}" "${LOG_DIR}"

migrate_beta4_runner_layout(){
  local old_root="${STORAGE_ROOT}/agents" old_request="${STATE_DIR}/agent-requests" old_reset="${STATE_DIR}/agent-resets" item id
  if [[ -d "${old_root}" ]]; then
    shopt -s nullglob
    for item in "${old_root}"/*; do
      [[ -d "${item}" ]] || continue
      id="$(basename "${item}")"
      if [[ ! -e "${RUNNER_ROOT}/${id}" ]]; then
        mv "${item}" "${RUNNER_ROOT}/${id}"
      fi
      if [[ -f "${RUNNER_ROOT}/${id}/.forgecore-agent.json" && ! -f "${RUNNER_ROOT}/${id}/.forgecore-runner.json" ]]; then
        sed 's/ForgeCoreAgent/ForgeCoreRunner/g' "${RUNNER_ROOT}/${id}/.forgecore-agent.json" > "${RUNNER_ROOT}/${id}/.forgecore-runner.json"
        rm -f "${RUNNER_ROOT}/${id}/.forgecore-agent.json"
      fi
    done
    shopt -u nullglob
    rmdir "${old_root}" 2>/dev/null || true
  fi
  if [[ -d "${old_request}" ]]; then
    find "${old_request}" -maxdepth 1 -type f -name '*.json' -exec mv -n {} "${REQUEST_DIR}/" \; 2>/dev/null || true
    rmdir "${old_request}" 2>/dev/null || true
  fi
  if [[ -d "${old_reset}" ]]; then
    find "${old_reset}" -maxdepth 1 -type f -name '*.json' -exec mv -n {} "${RESET_DIR}/" \; 2>/dev/null || true
    rmdir "${old_reset}" 2>/dev/null || true
  fi
  shopt -s nullglob
  for item in "${STATE_DIR}"/agent-*.json "${STATE_DIR}"/agent-*.pid; do
    id="$(basename "${item}")"
    mv -n "${item}" "${STATE_DIR}/${id/agent-/runner-}" 2>/dev/null || true
  done
  shopt -u nullglob
}
...
log(){
  printf '%s [runner-manager] %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$*" | tee -a "${LOG_DIR}/runner-manager.log"
}
...
state_file(){ printf '%s/runner-%s.json' "${STATE_DIR}" "$1"; }
pid_file(){ printf '%s/runner-%s.pid' "${STATE_DIR}" "$1"; }
...
start_runner(){
  ...
  id="$(slugify "${repo}")"
  dir="${RUNNER_ROOT}/${id}"
  log_file="${LOG_DIR}/runner-${id}.log"
  [[ -f "${dir}/.forgecore-runner.json" && -x "${dir}/run.sh" ]] || return 0
  running "${id}" && return 0
...
configure_request(){
  ...
  reg_log="${LOG_DIR}/runner-registration-${id}.log"
  stop_runner "${id}"

  if [[ "${replace}" == "true" ]]; then
    rm -rf "${dir}"
  elif [[ -f "${dir}/.forgecore-runner.json" ]]; then
    rm -f "${request}"
    start_runner "${repo}"
    return 0
  fi
  ...
  jq -n     --arg kind "ForgeCoreRunner"     --arg repository "${repo}"     --arg name "${name}"     --argjson createdEpoch "$(date +%s)"     '{kind:$kind,repository:$repository,name:$name,createdEpoch:$createdEpoch}'     > "${dir}/.forgecore-runner.json"
  chmod 600 "${dir}/.forgecore-runner.json" "${dir}/.runner" "${dir}/.credentials" 2>/dev/null || true
  rm -f "${request}"
...
reset_request(){
  ...
  if [[ -d "${dir}" && -f "${dir}/.runner" && -n "${token}" ]]; then
    set +e
    (cd "${dir}" && ./config.sh remove --unattended --token "${token}") >> "${LOG_DIR}/runner-registration-${id}.log" 2>&1
    set -e
  fi
  rm -rf "${dir}"
...
start_saved(){
  local meta repo
  shopt -s nullglob
  for meta in "${RUNNER_ROOT}"/*/.forgecore-runner.json; do
    repo="$(jq -r '.repository // empty' "${meta}" 2>/dev/null || true)"
    [[ "${repo}" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]] && start_runner "${repo}"
  done
  shopt -u nullglob
}
...
cleanup(){
  local pf pid
  shopt -s nullglob
  for pf in "${STATE_DIR}"/runner-*.pid; do
    pid="$(cat "${pf}" 2>/dev/null || true)"
